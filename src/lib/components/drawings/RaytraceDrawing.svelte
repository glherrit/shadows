<script lang="ts">
  ...
  import { genWaveFrontLine } from '$lib/erroranalysis'
  ...
  import LensSVG from './LensWithRaytrace.svelte'
  ...
  $: wfe = genWaveFrontLine(lens, source, 101)
  $: wfeValue = (wfe.max - wfe.min).toFixed(wfe.max - wfe.min > 9.99 ? 2 : 3)

  const outerWidth = 850
  const outerHeight = 600

  const topBoxHeight = 80

  // top left position data
  const topCenter = (topBoxHeight / 2).toFixed(0)
  const topBottom = topBoxHeight.toString()
...
>
  ...
  <rect
    ...
    width={outerWidth}
    height={outerHeight}
    style="fill: #EBF5FB; stroke: black; stroke-width: 3;"
  />
  ...
  <text class="small" text-anchor="middle" x={textPosX2[2]} y={textPosY1a}>
    {$_('drawing.titleLens')}
  </text>
  <text class="small" text-anchor="middle" x={textPosX2[2]} y={textPosY1b}>
    {$_('drawing.titleDiameter')}
  </text>
  <text class="small" text-anchor="middle" x={textPosX2[3]} y={textPosY1a}>
    {$_('drawing.titleBeam')}
  </text>
  <text class="small" text-anchor="middle" x={textPosX2[3]} y={textPosY1b}>
    {$_('drawing.titleDiameter')}
  </text>
  ...
  <text class="small2" text-anchor="middle" x={textPosX2[5]} y={textPosY2}>
    {$_('units.waves', { values: { num: wfeValue } })}
  </text>
  ...
  <LensSVG {lens} {source} {outerHeight} {outerWidth} />
</svg>

<div class="flex justify-end gap-4">
  <button class="btn-primary btn" type="button" on:click={handleCopyToClipboardClick}>
    {$_('buttons.copyToClipboard')}
  </button>
  <button class="btn-primary btn" type="button" on:click={handleSaveSVGClick}>
    {$_('buttons.saveToSVG')}
  </button>
</div>
